...
void A_star_planner<NodeType, CostType>::init(
    GenericSearchGraphDescriptor<NodeType, CostType> *theEnv_p,
    bool createHashAndHeap) {
  GraphNode_p thisGraphNode;

  if (theEnv_p && createHashAndHeap)
    GenericPlannerInstance.init(*theEnv_p, heapKeyCount); // This initiates the
                                                          // graph, hash and
                                                          // heap of the generic
                                                          // planner
  else if (theEnv_p)
  ...
  // Init graph, clear the heap and clear stored paths just in case they not
  // empty due to a previous planning
  ...
  for (int a = 0; a < GraphDescriptor->SeedNodes.size(); a++) {
    ...
    if (!thisGraphNode->initiated) {
      thisGraphNode->f = _heapFun(
          thisGraphNode->n, (CostType)0.0,
          GraphDescriptor->_getHeuristicsToTarget(thisGraphNode->n), a);
      ...
      if (!GraphDescriptor->_isAccessible(thisGraphNode->n)) {
        printf("ERROR (A_star): At least one of the seed nodes is not "
               "accessible!");
        exit(1);
      } else
        thisGraphNode->plannerVars.accessible = true;

      thisGraphNode->initiated =
          true; // Always set this when other variables have already been set
    }
...
template <class NodeType, class CostType>
void A_star_planner<NodeType, CostType>::clearLastPlanAndInit(
    GenericSearchGraphDescriptor<NodeType, CostType> *theEnv_p) {
...
template <class NodeType, class CostType>
CostType A_star_planner<NodeType, CostType>::_heapFun(NodeType &n, CostType g,
                                                      CostType h, int s) {
  if (heapFun_fp)
...
void A_star_planner<NodeType, CostType>::plan(void) {
  ...
  while (!heap->empty()) {
    ...
    if (ProgressShowInterval > 0) {
      if (expandcount % ProgressShowInterval == 0) {
        ...
        printf(
            "Number of states expanded: %d. Heap size: %d. Time elapsed: %f "
            "s.\n",
            expandcount, heap->size(),
    ...
    if (event_NodeExpanded_g)
      ...
      (thisGraphNode->n.*
       event_NodeExpanded_nm)(thisGraphNode->plannerVars.g, thisGraphNode->f,
                              thisGraphNode->plannerVars.seedLineage);
#endif
    ...
    if (GraphDescriptor->_stopSearch(thisGraphNode->n)) {
      ...
      if (ProgressShowInterval > 0) {
        if (timediff >= 0.0)
          timediff = ((float)(clock() - startclock)) / ((float)CLOCKS_PER_SEC);
        printf(
            "Stopping search!! Number of states expanded: %d. Heap size: %d. "
            "Time elapsed: %f s.\n",
            expandcount, heap->size(),
            ((timediff >= 0.0) ? timediff : difftime(time(NULL), startsecond)));
      }
    ...
    if (GraphDescriptor->_storePath(thisGraphNode->n)) {
      ...
      if (timediff >= 0.0)
        timediff = ((float)(clock() - startclock)) / ((float)CLOCKS_PER_SEC);
      printf(
          "Stored a path!! Number of states expanded: %d. Heap size: %d. Time "
          "elapsed: %f s.\n",
          expandcount, heap->size(),
          ((timediff >= 0.0) ? timediff : difftime(time(NULL), startsecond)));
#endif
    ...
    if (thisGraphNode->successors
            .empty()) // Successors were not generated previously
    {
      GraphDescriptor->_getSuccessors(thisGraphNode->n, &thisNeighbours,
                                      &thisTransitionCosts);
      thisGraphNode->successors.init(thisNeighbours.size());
      for (a = 0; a < thisNeighbours.size(); a++)
        thisGraphNode->successors.set(a, hash->getNodeInHash(thisNeighbours[a]),
                                      thisTransitionCosts[a]);
    }
#if _YAGSBPL_A_STAR__PRINT_VERBOSE
    ...
    for (a = 0; a < thisGraphNode->successors.size(); a++) {
      thisNeighbourGraphNode =
          thisGraphNode->successors.getLinkSearchGraphNode(a);
      ...
      if (!thisNeighbourGraphNode->initiated) {
        thisNeighbourGraphNode->plannerVars.accessible =
            GraphDescriptor->_isAccessible(thisNeighbourGraphNode->n);
        if (thisNeighbourGraphNode->plannerVars.accessible) {
          ...
          thisNeighbourGraphNode->plannerVars.g =
              thisGraphNode->plannerVars.g + thisTransitionCost;
          thisNeighbourGraphNode->f = _heapFun(
              thisNeighbourGraphNode->n, thisNeighbourGraphNode->plannerVars.g,
              GraphDescriptor->_getHeuristicsToTarget(
                  thisNeighbourGraphNode->n),
              thisGraphNode->plannerVars.seedLineage);
          // thisNeighbourGraphNode->plannerVars.g +
          // subopEps * GraphDescriptor->_getHeuristicsToTarget(
          // thisNeighbourGraphNode->n );
        ...
        thisNeighbourGraphNode->initiated =
            true; // Always set this when other variables have already been set
#if _YAGSBPL_A_STAR__HANDLE_EVENTS
        if (event_SuccUpdated_g)
          event_SuccUpdated_g(
              thisGraphNode->n, thisNeighbourGraphNode->n, thisTransitionCost,
              thisNeighbourGraphNode->plannerVars.g, thisNeighbourGraphNode->f,
              thisNeighbourGraphNode->plannerVars.seedLineage);
        else if (event_SuccUpdated_nm)
          (thisGraphNode->n.*event_SuccUpdated_nm)(
              thisNeighbourGraphNode->n, thisTransitionCost,
              thisNeighbourGraphNode->plannerVars.g, thisNeighbourGraphNode->f,
              thisNeighbourGraphNode->plannerVars.seedLineage);
#endif
        continue;
      ...
      if (!thisNeighbourGraphNode->plannerVars.accessible ||
          thisNeighbourGraphNode->plannerVars.expanded)
        continue;
  ...
  if (ProgressShowInterval > 0 && heap->empty())
    printf("Stopping search!! Heap is empty. Number of states expanded: %d. "
           "Heap size: %d. Time elapsed: %f s.\n",
           expandcount, heap->size(),
           ((timediff >= 0.0) ? timediff : difftime(time(NULL), startsecond)));
#endif
}
...
template <class NodeType, class CostType>
std::vector<CostType>
A_star_planner<NodeType, CostType>::getPlannedPathCosts(void) {
...
A_star_planner<NodeType, CostType>::getPlannedPaths(void) {
...
template <class NodeType, class CostType>
A_star_variables<CostType>
A_star_planner<NodeType, CostType>::getNodeInfo(NodeType n) {
