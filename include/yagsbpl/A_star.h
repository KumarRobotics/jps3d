...
#define _YAGSBPL_A_STAR__PRINT_VERBOSE                                         \
  0 // requires 'void print(std::string pre)' as a member function of the node
#define _YAGSBPL_A_STAR__HANDLE_EVENTS 1

template <class CostType> class A_star_variables {
public:
  CostType g;
  bool expanded;   // Whether in closed list or not
  bool accessible; // Since the environment is assumed to to change, each node
                   // has fixed accessibility
...
template <class NodeType, class CostType> class A_star_planner {
public:
  ...
  GenericPlanner<NodeType, CostType, PlannerSpecificVariables>
      GenericPlannerInstance;
  // Re-mapping of generic planner variables for ease of use (coding
  // convenience)
...
// Optional event handlers - Pointers to function that get called when an event
// take place
#if _YAGSBPL_A_STAR__HANDLE_EVENTS
  // Node 'n' is expanded. This can also be handeled by 'stopSearch'.
  void (*event_NodeExpanded_g)(NodeType n, CostType gVal, CostType fVal,
                               int seedLineage);
  void (NodeType::*event_NodeExpanded_nm)(CostType gVal, CostType fVal,
                                          int seedLineage);
  // Successor 'nn' is in open list and is just initiated or is updated
  void (*event_SuccUpdated_g)(NodeType n, NodeType nn, CostType edgeCost,
                              CostType gVal, CostType fVal, int seedLineage);
  void (NodeType::*event_SuccUpdated_nm)(NodeType nn, CostType edgeCost,
                                         CostType gVal, CostType fVal,
                                         int seedLineage);
#endif
  ...
  void setParams(double eps = 1.0, int heapKeyCt = 20,
                 int progressDispInterval = 10000) // call to this is optional.
  {
    subopEps = eps;
  ...
  void init(GenericSearchGraphDescriptor<NodeType, CostType> *theEnv_p = NULL,
            bool createHashAndHeap = true);
  void init(GenericSearchGraphDescriptor<NodeType, CostType> theEnv) {
    init(&theEnv);
  } // for version compatability
  void plan(void);

  // Clear the last plan, but not the hash table. Must be called after at least
  // one initialization.
  //   Use this if you intend to re-plan with different start/goal,
  //   and/or if isAccessible is changed such that the new planning is being
  //   done in a sub-graph of the previous graph,
  //   Won't give correct result if cost function has changed or the new graph
  //   has new edges attached to nodes explored in old graph.
  void clearLastPlanAndInit(
      GenericSearchGraphDescriptor<NodeType, CostType> *theEnv_p = NULL);

  // Planner output access: ( to be called after plan(), and before destruction
  // of planner )
...
// Other variables for querying progress of planning process from other
// functions
