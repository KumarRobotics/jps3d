#include <jps3d/planner/graph_search_3d_util.h>

using namespace JPS;

GraphSearch3DUtil::GraphSearch3DUtil(bool verbose) {
  planner_verbose_ = verbose;
  if(planner_verbose_)
    printf(ANSI_COLOR_CYAN "GRAPHSEARCH3D PLANNER VERBOSE ON\n" ANSI_COLOR_RESET);
...
bool GraphSearch3DUtil::plan(const Vec3f &start, const Vec3f &goal, decimal_t eps, bool use_jps) {
  if(planner_verbose_){
  ...
  if (map_util_->isOutSideXYZ(goal_int, 0) ||
      ...
      map_util_->isOutSideXYZ(goal_int, 2))
    goal_outside_ = true;
  else
  {
  ...
  for( int z = 0; z < dim(2); ++z)
    for( int y = 0; y < dim(1); ++y)
      ...
      {
        if(map_util_->isOccupied(Vec3i(x,y,z)))
          cmap[x+y*dim(0)+z*dim(0)*dim(1)] = 1;
        else
          cmap[x+y*dim(0)+z*dim(0)*dim(1)] = 0;
      }

  graph_search_ = std::make_shared<JPS::GraphSearch>(cmap.data(), dim(0), dim(1), dim(2), eps, planner_verbose_);
